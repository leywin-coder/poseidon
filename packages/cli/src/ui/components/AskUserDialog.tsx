    ...
    (itemValue: OptionItem) => {
      if (question.multiSelect) {
        ...
        if (itemValue.type === 'option') {
          ...
          // In single select, selecting other submits it if it has meaningful text
          // (minimum 3 chars to avoid accidental single/double-key submissions)
          if (customOptionText.trim().length >= 3) {
            onAnswer(
              expandPastePlaceholders(
                customOptionText,
                customBuffer.pastedContent,
