...
import type { ModelDefinition } from '@google/gemini-cli-core';
...
export function ModelDialog({ onClose }: ModelDialogProps): React.JSX.Element {
  ...
  const [view, setView] = useState<'main' | 'manual'>(() => {
    // For non-Google providers, skip the "auto" main view and go straight
    // to the model list so the user sees the discovered models immediately.
    if (config?.getActiveProviderId() && config.getActiveProviderId() !== 'google') {
      return 'manual';
    }
  ...
  useEffect(() => {
    async function checkAccess() {
      if (!config) return;
      // For non-Google providers, we're already in manual view — nothing to check.
      if (config.getActiveProviderId() !== 'google') return;
  ...
  // Discover runtime models for non-Google providers (e.g., OpenAI-compatible).
  useEffect(() => {
    if (!config || !config.getExperimentalDynamicModelConfiguration?.()) return;
    ...
    if (!provider || !provider.discoverModels) return;
    ...
    provider.discoverModels(baseUrl, apiKey).then((discovered: Record<string, ModelDefinition>) => {
      // Mark all discovered models as visible for the dialog
      for (const key of Object.keys(discovered)) {
        discovered[key].isVisible = true;
      }
      config.getModelConfigService().registerRuntimeModelDefinitions(discovered);
      setDiscoveryVersion((v) => v + 1);
    }).catch(() => {
      // Discovery failed silently — models may be empty or user will type manually
    });
  }, [config]);
  ...
  const mainOptions = useMemo(() => {
    void discoveryVersion; // Force refresh after model discovery
    // --- DYNAMIC PATH ---
    if (
      config?.getExperimentalDynamicModelConfiguration?.() === true &&
      config.getModelConfigService
      ...
      const allOptions = config
        ...
        .getAvailableModelOptions({
          providerId: config.getActiveProviderId(),
