...
describe('useAuth', () => {
  ...
  describe('useAuthCommand', () => {
    ...
    it('should proceed without auth if no auth type is selected', async () => {
      const { result } = await renderHook(() =>
        useAuthCommand(createSettings(undefined), mockConfig),
      );

      // Auth is now optional — no error, immediately Authenticated
      expect(result.current.authError).toBeNull();
      expect(result.current.authState).toBe(AuthState.Authenticated);
    });

    it('should proceed without auth if no auth type is selected but env key exists', async () => {
      process.env['GEMINI_API_KEY'] = 'env-key';
      const { result } = await renderHook(() =>
        useAuthCommand(createSettings(undefined), mockConfig),
      );

      // Auth is optional even when env key exists
      expect(result.current.authError).toBeNull();
      expect(result.current.authState).toBe(AuthState.Authenticated);
    });
