...
export function AuthDialog({
  ...
  let initialAuthIndex = items.findIndex((item) => {
    ...
    // Default to Gemini API Key as the least intrusive option,
    // instead of requiring a Google login.
    return item.value === AuthType.USE_GEMINI;
  });
  if (settings.merged.security.auth.enforcedType) {
    initialAuthIndex = 0;
    ...
    (key) => {
      if (key.name === 'escape') {
        ...
        if (settings.merged.security.auth.selectedType === undefined) {
          // Auth is optional — dismiss dialog and continue without auth.
          setAuthState(AuthState.Unauthenticated);
          return true;
        }
        // eslint-disable-next-line @typescript-eslint/no-floating-promises
    ...
    return (
      ...
      >
        <Text color={theme.text.primary}>
          Logging in with Google... Restarting Poseidon CLI to continue.
        </Text>
      </Box>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        <Box marginTop={1}>
          <Text color={theme.text.primary}>
            Terms of Services and Privacy Notice for Poseidon CLI
          </Text>
        </Box>
        <Box marginTop={1}>
