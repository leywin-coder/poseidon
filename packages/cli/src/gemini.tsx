...
export async function main() {
  ...
  // Restore saved provider from settings (e.g. openai-compatible with baseUrl)
  const savedProvider = settings.merged.activeProvider as
    | { id?: string; name?: string }
    | undefined;
  const restoreSavedProvider = async (cfg: Config) => {
    if (savedProvider?.id && savedProvider.id !== 'google') {
      try {
        const savedProviders = settings.merged.openaiCompatibleProviders as
          | Record<string, { baseUrl?: string; apiKey?: string; defaultModel?: string }>
          | undefined;
        const providerEntry =
          savedProvider.name && savedProviders
            ? savedProviders[savedProvider.name]
            : undefined;
        await cfg.setProvider(
          savedProvider.id,
          providerEntry?.apiKey,
          providerEntry?.baseUrl,
        );
        if (providerEntry?.defaultModel) {
          cfg.setModel(providerEntry.defaultModel, false);
        }
      } catch (e) {
        // Non-fatal — fall back to google if restore fails
      }
    }
  };
  await restoreSavedProvider(partialConfig);
  ...
  // may have side effects.
  {
    ...
    // Restore saved provider on the real config object (same as partialConfig above).
    await restoreSavedProvider(config);
