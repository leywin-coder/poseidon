...
export async function parseArguments(
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('poseidon')
    .usage(
      'Usage: poseidon [options] [command]\n\nPoseidon CLI - Defaults to interactive mode. Use -p/--prompt for non-interactive (headless) mode.',
    )
    .option('isCommand', {
      type: 'boolean',
      hidden: true,
  ...
  yargsInstance
    .command('$0 [query..]', 'Launch Poseidon CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
