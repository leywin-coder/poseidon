...
/**
 * Persist the active OpenAI-compatible provider so it is restored on the
 * next startup. Saves both the provider entry under `openaiCompatibleProviders`
 * and the `activeProvider` pointer.
 */
export function saveActiveProvider(
  loadedSettings: LoadedSettings,
  providerName: string,
  baseUrl: string,
  defaultModel: string,
  ...
  try {
    // Normalize baseUrl: strip trailing /v1 since the code always appends /v1/...
    const normalizedUrl = baseUrl
      ? baseUrl.replace(/\/+$/, '').replace(/\/v1$/, '')
      : baseUrl;
    const mergedMap = loadedSettings.merged as Record<string, unknown>;
    const existing = (mergedMap['openaiCompatibleProviders'] ?? {}) as Record<string, unknown>;
    const updated = {
      ...existing,
      [providerName]: {
    ...
    loadedSettings.setValue(SettingScope.User, 'activeProvider', {
    ...
    coreEvents.emitFeedback(
      'error',
      `Failed to save provider configuration: ${detailedErrorMessage}`,
      error,
    );
  }
}
