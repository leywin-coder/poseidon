...
// Export multi-provider support
export * from './core/llmProvider.js';
export * from './core/llmProviderRegistry.js';
export * from './core/googleProvider.js';
export * from './core/openaiProvider.js';
export * from './core/providers/openaiLlmProvider.js';
export * from './core/providers/openaiContentGenerator.js';
export * from './core/providerMessageTranslator.js';
export * from './core/openaiApiServer.js';

import { LlmProviderRegistry } from './core/llmProviderRegistry.js';

/**
 * Register built-in LLM providers. Uses dynamic imports to avoid circular
 * ESM dependency issues (the provider classes import from contentGenerator.ts
 * which transitively re-exports from this file).
 *
 * Returns a promise that resolves once all providers are registered.
 * Call this once at startup before any `LlmProviderRegistry.get()` or
 * `Config.setProvider()` calls.
 */
export async function initProviders(): Promise<void> {
  const [
    ...
    import('./core/googleProvider.js'),
    import('./core/openaiProvider.js'),
    import('./core/providers/openaiLlmProvider.js'),
  ]);

  LlmProviderRegistry.register(new GoogleLlmProvider());
  LlmProviderRegistry.register(new OpenAILlmProvider());
  LlmProviderRegistry.register(new OpenAICompatibleLlmProvider());
}

// Kick off registration immediately on module load (fire-and-forget).
// For cases where order matters (e.g. startup provider restore),
// import and await initProviders() explicitly.
initProviders().catch((err) => {
  console.error('[gemini-cli-core] Failed to register built-in providers:', err);
});
