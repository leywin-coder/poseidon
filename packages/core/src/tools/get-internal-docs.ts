...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Poseidon CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Poseidon CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Poseidon CLI's internal documentation.
