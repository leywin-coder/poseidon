...
export type ContentGeneratorConfig = {
  ...
  /**
   * @internal
   * Config instance passed by Config.setProvider() for use by providers
   * that need it (e.g. Google provider).
   */
...
/**
 * Create a content generator using the LLM provider registry.
 *
 * This is the entry point for multi-provider support. When a non-Google
 * provider is active, this function delegates to the registered provider
 * to create a client.
 *
 * @param providerId - The provider ID (e.g. 'google', 'openai').
 * @param config - Provider configuration including API key and options.
 * @returns A ContentGenerator that wraps the provider's client.
 */
export async function createLlmProviderContentGenerator(
  providerId: string,
  config: ContentGeneratorConfig,
): Promise<ContentGenerator> {
  ...
  if (!provider) {
    throw new Error(
      `Unknown provider: "${providerId}". ` +
        `Available providers: ${LlmProviderRegistry.getAll().map((p) => p.id).join(', ') || 'none'}`,
    );
  }

  const providerClient = await provider.createClient({
    providerId,
  ...
  // Wrap in LoggingContentGenerator for telemetry consistency
  const { LoggingContentGenerator } = await import('./loggingContentGenerator.js');
  return new LoggingContentGenerator(providerClient as unknown as ContentGenerator, config._internalConfig as Config);
}
