...
export class GeminiClient {
  ...
  private async *processTurn(
    ...
    const modelConfigKey: ModelConfigKey = {
      model: modelToUse,
      isChatModel: true,
      providerId: this.config.getActiveProviderId(),
    };
  ...
  async generateContent(
    ...
    // Inject active providerId into the config key if not already set
    const keyWithProvider: ModelConfigKey = {
      ...modelConfigKey,
      providerId: modelConfigKey.providerId ?? this.config.getActiveProviderId(),
    };
    const desiredModelConfig =
      this.config.modelConfigService.getResolvedConfig(keyWithProvider);
    let {
    ...
    try {
      ...
      } = applyModelSelection(this.config, keyWithProvider);
