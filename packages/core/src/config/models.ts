...
export function getAutoModelDescription(
  ...
  const flashModel = hasAccessToPreview
    ? useGemini3_5Flash
      ...
      : PREVIEW_GEMINI_FLASH_MODEL
    : DEFAULT_GEMINI_FLASH_MODEL;
  return `Let Poseidon CLI decide the best model for the task: ${getDisplayString(proModel)}, ${getDisplayString(flashModel)}`;
}
...
export function resolveModel(
  ...
  // For non-Google providers (e.g. openai-compatible), skip Gemini-specific
  // alias resolution and return the model name verbatim so the provider's own
  // routing handles it (e.g. "auto" is passed as-is to an OpenAI-compatible proxy).
  if (config && 'getActiveProviderId' in config) {
    const providerId = (config as { getActiveProviderId(): string }).getActiveProviderId();
