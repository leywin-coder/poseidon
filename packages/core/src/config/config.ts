...
export class Config implements McpContext, AgentLoopContext {
  ...
  private activeProviderId = 'google';
  ...
  private async _initialize(): Promise<void> {
    ...
    if (!this.interactive || this.acpMode) {
      // In non-interactive (headless) mode we still want MCP tools available,
      // but slow MCP servers (e.g. npx-based extensions) shouldn't block a
      // simple -p prompt for more than a few seconds.
      const MCP_HEADLESS_TIMEOUT_MS = 5_000;
      const timeout = new Promise<void>((resolve) =>
        setTimeout(() => {
          debugLogger.warn(
            `[MCP] Non-interactive startup: MCP initialization timed out after ${MCP_HEADLESS_TIMEOUT_MS}ms. Continuing without waiting.`,
          );
          resolve();
        }, MCP_HEADLESS_TIMEOUT_MS),
      );
      await Promise.race([this.mcpInitializationPromise, timeout]);
    }
   ...
   *
   * This re-creates the content generator using the provider registry.
   * The provider must be registered in LlmProviderRegistry.
   *
   * @param providerId - The provider to switch to (e.g. 'google', 'openai').
   * @param apiKey - Optional API key for the new provider.
  ...
  async setProvider(
    ...
    const { LlmProviderRegistry } = await import('../core/llmProviderRegistry.js');
    const provider = LlmProviderRegistry.get(providerId);

    if (!provider) {
      throw new Error(
        `Unknown provider: "${providerId}". ` +
          `Available: ${LlmProviderRegistry.getAll().map((p) => p.id).join(', ')}`,
      );
    }

    // Determine auth type for the new provider
    ...
    // Create the new content generator via the provider
    const { createLlmProviderContentGenerator } = await import(
      '../core/contentGenerator.js'
    );

    const newGenerator = await createLlmProviderContentGenerator(
      providerId,
      newConfig,
    );

    // Update state
    this.contentGenerator = newGenerator;
    this.contentGeneratorConfig = newConfig;
    this.activeProviderId = providerId;

    // Re-initialize BaseLlmClient with the new generator
  ...
  async refreshAuth(
    ...
    // If a non-Google provider is active (e.g. openai-compatible), skip Google
    // auth entirely — refreshAuth rebuilds the content generator using Google's
    // API, which would overwrite the active third-party provider.
    if (this.activeProviderId !== 'google') {
      return;
    }
    ...
    // Pass Config instance for providers that need it
    newContentGeneratorConfig._internalConfig = this;
