...
export interface ModelConfigKey {
  ...
  /**
   * The provider ID to scope model resolution to.
   * When specified, only models from the given provider are considered.
   */
  providerId?: string;
}
...
export interface ModelDefinition {
  ...
  family?: string; // The model family, e.g. 'gemini-3' | 'gpt-4' | 'o1'
  isPreview?: boolean;
   ...
   * Models are scoped to their provider for filtering and routing.
...
export class ModelConfigService {
   ...
   */
  /**
   * Register dynamically discovered model definitions at runtime.
   * These are merged with static definitions and take precedence for matching keys.
   */
  registerRuntimeModelDefinitions(defs: Record<string, ModelDefinition>): void {
    Object.assign(this.runtimeModelDefinitions, defs);
  }
  ...
  getAvailableModelOptions(context: ResolutionContext): Array<{
    ...
    const staticDefinitions = this.config.modelDefinitions ?? {};
    // Merge runtime definitions on top of static definitions (runtime wins).
    const definitions = { ...staticDefinitions, ...this.runtimeModelDefinitions };
    ...
    // Filter function: only include models from the active provider.
    // Models without providerId are treated as 'google' (backwards compat).
    const matchesProvider = (m: ModelDefinition): boolean => {
      if (!providerId) return true; // No filter — show all
      const modelProvider = m.providerId ?? 'google';
    ...
    const mainOptions = Object.entries(definitions)
      .filter(([_, m]) => {
        ...
        if (!matchesProvider(m)) return false;
        return true;
      })
      .map(([id, m]) => {
  ...
  getModelDefinition(modelId: string): ModelDefinition | undefined {
    // Runtime definitions take precedence.
    const runtimeDef = this.runtimeModelDefinitions?.[modelId];
    if (runtimeDef) {
      return runtimeDef;
    }
  ...
  getModelDefinitions(): Record<string, ModelDefinition> {
    return { ...this.config.modelDefinitions, ...this.runtimeModelDefinitions };
  }
