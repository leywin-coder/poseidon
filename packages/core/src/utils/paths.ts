...
export const POSEIDON_DIR = '.poseidon';
/** @deprecated Use POSEIDON_DIR instead */
export const GEMINI_DIR = POSEIDON_DIR;
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.json';
export const TRUSTED_FOLDERS_FILENAME = 'trustedFolders.json';

/**
 * Returns the home directory.
 * If POSEIDON_CLI_HOME environment variable is set, it returns its value.
 * Otherwise, it returns the user's home directory.
 */
export function homedir(): string {
  const envHome = process.env['POSEIDON_CLI_HOME'];
  if (envHome) {
    return envHome;
  }
  return os.homedir();
