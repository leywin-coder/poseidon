...
export class NumericalClassifierStrategy implements RoutingStrategy {
  ...
  async route(
    ...
    try {
      ...
      // Skip classifier for non-Google providers — the classifier calls
      // baseLlmClient which is Google's API, so it would fail (or hit Google)
      // even when the user has switched to an OpenAI-compatible provider.
      if (config.getActiveProviderId() !== 'google') {
        return null;
      }
